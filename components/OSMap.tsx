...
function buildHTML() {
  return (
    ...
    '</style></head><body>' +
    '<div id="dbg" style="position:fixed;top:0;left:0;right:0;background:#000;color:#0f0;padding:2px 6px;font-size:10px;z-index:99999;font-family:monospace;">init</div>' +
    ...
    'function setCenter(lat,lon,zoom){map.setView([lat,lon],zoom||17);}' +
    'function fitBounds(){' +
    ...
    'else{currentMarker=L.circleMarker([lat,lon],{radius:7,color:\"#00E5CC\",fillColor:\"#ffffff\",fillOpacity:1,weight:2}).addTo(map);}' +
    'if(coords.length===1){map.setView([lat,lon],17);}' +
    'else{' +
    'if(!polyline){polyline=L.polyline(coords,{color:\"#00E5CC\",weight:5,opacity:0.9,lineCap:\"round\",lineJoin:\"round\"}).addTo(map);}' +
    'else{polyline.addLatLng([lat,lon]);}}}' +
